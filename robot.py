...
def valid_command(command):
    ...
    return command_name.lower() in valid_commands and (len(arg1) == 0 or is_int(arg1) or arg1 == 'silent')
...
def do_replay(robot_name, history, is_silent=False):
...
def handle_command(robot_name, command, history):
    ...
    if command_name == 'off':
        ...
        (do_next, command_output) = do_replay(robot_name, history)
...
def robot_start():
    ...
    while handle_command(robot_name, command, history):
        if 'replay' not in command:
            history = keep_history(command, history)
        command = get_command(robot_name)
